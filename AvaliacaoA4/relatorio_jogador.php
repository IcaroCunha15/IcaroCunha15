...
if(isset($_GET['delete_id'])){
    ...
    $query = 'DELETE FROM tb_jogador WHERE id = :delete_id';
    $retorno = $conexao->prepare($query);
                $retorno->bindParam(':delete_id', $delete_id);
                $retorno->execute();

                header("Location: relatorio_jogador.php");
                exit();
}
?>
<!DOCTYPE html>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-dark bg-primary">
        <div class="container-fluid">
            <a class="navbar-brand">Relatório de funcionários</a>
    ...
    <div class="m-5">
        <table class="table table-striped">
            ...
            <tbody>
            <?php
                foreach ($lista as $jogador) {
                    echo "<tr class=\"text-start\">";
                    echo "<td>" . $jogador['id'] . "</td>";
                    echo "<td>" . $jogador['nome'] . "</td>";
                    echo "<td>" . $jogador['email'] . "</td>";
                    echo "<td>" . $jogador['telefone'] . "</td>";
                    echo "<td>" . $jogador['Esporte'] . "</td>";;
                    echo "<td>" . $jogador['Modalidade'] . "</td>";                  
                    echo "</tr>";
                    echo "<td>
                    <a class='btn btn-sm btn-danger' href='?delete_id={$jogador['id']}'>
